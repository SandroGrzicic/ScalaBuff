...
object ScalaBuff {
     ...
     */
    def apply(resourcePath: String)(implicit settings: Settings) = fromResourcePath(resourcePath)

    /**
     * Runs ScalaBuff on the specified resource path (file path or URL) and returns the resulting Scala class.
     * If the encoding is not specified, it defaults to either UTF-8 (if available) or the platform default charset.
     */
    def fromResourcePath(resourcePath: String)(implicit settings: Settings): ScalaClass = {
    ...
    def maybeShow(msg: String)(implicit settings: Settings) = if (settings.verbose) println(msg)
    ...
    def main(args: Array[String]) {
        args match {
            ...
            case arguments =>
                val (rawSettings, paths) = arguments.toSeq.partition(_.startsWith("-"))
                ...
                val protoFiles = paths match {
                    ...
                    case files =>
                        files map { f =>
                            ...
                            findProto(parsedSettings.importDirectories.head, parsedSettings.importDirectories.tail)
                        } filter { f => f.isDefined } map (_.get)
                }

                for (path <- protoFiles) {
                    maybeShow("Processing: " + path)
                    try {
